...
const UserProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const response = await fetch(
        "https://simple-api-backend-nodejs-express-fs.onrender.com/api/auth/login",
        {
          method: "POST",
  ...
  const register = async (email, password) => {
    try {
      const response = await fetch(
        "https://simple-api-backend-nodejs-express-fs.onrender.com/api/auth/register",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, password }),
  ...
  const profile = async () => {
    ...
    if (token) {
      ...
      try {
        const response = await fetch(
          "https://simple-api-backend-nodejs-express-fs.onrender.com/api/auth/me",
          {
            headers: {
  ...
  return (
    <UserContext.Provider
      value={{ logout, login, auth, register, profile, user }}
    >
      {children}
    </UserContext.Provider>
  );
